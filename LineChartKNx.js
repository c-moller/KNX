...
LoadItemNames: function () {
    ...
    function readItems(doc) {
        var jsonDoc, i, j, txt, arrayDoc, index;
        var nameUnits = [];
        var units = [];
        var labelUnits = [];
        var linkUnits = [];
...
/*RefreshData: function(){
    ...
    switch (period){
        case 'h': timer=60000; break;
        case '4h': timer=60000; break;
        case '8h': timer=60000; break;
        case '12h': timer=120000; break;
        case 'D': timer=240000; break;
        case '3D': timer=900000; break;
        case 'W': timer=undefined; break;
        case '2W': timer=undefined; break;
        case 'M': timer=undefined; break;
        case '2M': timer=undefined; break;
        case '4M': timer=undefined; break;
        case 'Y': timer=undefined; break;
        default: timer=60000; break;
    }
    if (timer!==undefined){
...
loadGraphs: function(){
    ...
    function readItems(doc) {
        ...
        for (var i = 0; i < jsonLen; i++) {
            if (jsonDoc[i].stateDescription !== undefined) {
                itemList.nameUnits.push(jsonDoc[i].name);
                itemList.unitName.push(jsonDoc[i].unit);
                itemList.units.push(jsonDoc[i].stateDescription.pattern);
                itemList.labelUnits.push(jsonDoc[i].label);
                itemList.linkUnits.push(jsonDoc[i].link);
            } 
        }
        sortUnit(itemList.nameUnits, itemList.units, itemList.labelUnits, itemList.linkUnits, itemList.unitName);
        console.log("Loaded realKNX items")
    }
    var object = this;
    //Sort variables per unit type
    function sortUnit(seriesName, unit, label, links, unitName) {
            ...
            for (i = 0; i < unitLen; i++) {
                if (unitString == "All" && roomSelectString == "All") {
                    ...
                    if (unitName[i] !== undefined) {
                        if (unitName[i] == "") {
                            object.ylabel.push("ON/OFF");
                        } else {
                            object.ylabel.push(unitName[i]);
                        }
                    }
                    ...
                    if(unitName[i] !== undefined){
                    	if (unitName[i].toUpperCase() == unitString.toUpperCase()) {
                      	  object.seriesName.push(seriesName[i]);
                    	    object.label.push(label[i]);
                    	    object.link.push(links[i]);
                    	    object.ylabel.push(unitName[i]);
	    ...
	    function DrawPie(state, colorScheme,label,unit) {
	        ...
	        function ChartPie() {
	            ...
	            var options = {
	                ...
	                colors: [colorScheme[0],colorScheme[1],colorScheme[2],colorScheme[3],colorScheme[4],colorScheme[0],colorScheme[2],colorScheme[1],colorScheme[4],colorScheme[3]]
	            };
